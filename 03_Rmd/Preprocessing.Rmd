...
Investigate if there are other such cases. -> there are, the respective cases are extracted below. There are some cases, where there are different pos tags and some, where there are multiple entries with different pos tags. For the second case, the frequency will be extracted manually.
...
```{r}
# Create a subset of eye_table with the desired condition
...
#### VADER
```{r call_vader, eval=FALSE, message=FALSE, warning=FALSE}
