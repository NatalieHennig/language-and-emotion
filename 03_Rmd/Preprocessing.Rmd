...
```{r get_frequency}
column_names <- c("sort-order", "frequency", "word", "word-class")
frequency_df <- read_delim(here("01_data", "lemma.al.txt"), delim = " ", col_names = column_names)
```
...
```{r}
# TODO: Map pos from BNC frequency to POS in eye_data
# TODO: Group lemma & pos and word to get frequencies because some words occur multiple times but contain different word classes
```
example: 
3341 2139 above a
942 10719 above adv
...
```{r add_frequency}
frequency_df <- frequency_df %>% select("word", "frequency", "word-class")

#eye_table <- left_join(eye_table, frequency_df, by = c("lemma" = "word")) 
result <- left_join(eye_table, frequency_df, by = c("lemma" = "word")) %>%
...
```{r remove_dataframes}
#rm("merger", "POS", "texts_listed", "vader_scores", "joint.data", "text_df", "text_in")
```
