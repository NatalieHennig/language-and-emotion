...
frequency_whole <- read.csv(here("05_output", "mapped_pos_frequency.csv"), stringsAsFactors = FALSE, colClasses = c(Word = "character"))
...
- **Case 3:** Different spellings / others
- **Case 4:** Tags from the frequency table diverge from the ones in *eye_table*
...
```{r case_3}
eye_table$frequency[eye_table$token == "one’s"] <- 2
eye_table$frequency[eye_table$token == "today’s"] <- 25765
eye_table$frequency[eye_table$token == "Vinci’s"] <- 59
eye_table$frequency[eye_table$token == "kinesthetic"] <- 6
eye_table$frequency[eye_table$token == "“lead”"] <- 14324
eye_table$frequency[eye_table$token == "true"] <- 17718
```
