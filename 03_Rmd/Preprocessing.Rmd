...
```{r individual_texts, eval=FALSE, include=FALSE}
...
First, the columns *textnr*, *tokenindex*, and *token* for each text is extracted. They serve as a key, so that they can be reassigned with the polarity scores to the main table.
```{r export_text_column}
...
```{python }
import pandas as pd
df = pd.read_csv("/Users/nataliehennig/Documents/language-and-emotion/05_output/text1_tokens.csv")
```
