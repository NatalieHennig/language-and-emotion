...
text8$ANEW <- text8$token |> textstat_valence(dictionary = data_dictionary_ANEW["pleasure"]) 
text8 <- unpack(text8, cols = c(ANEW)) 
text8 = subset(select(text8, -c(doc_id)))

```
Words that are not in the dictionary are automatically assigned a value of 0.00 which needs to be taken into consideration.
These words must be declared missing.
