...
article <- article %>%
  ...
  mutate(medianL1L2 = median(dur), .after = dur, .groups = 'drop')
```

```{r percentage_0}
(sum(article$medianL1L2==0)/length(article$medianL1L2))*100
```
...
```{r RQ2}
summary(lm(medianL1L2 ~ word_length + word_frequency + punctuation + valency + 
             exposure_EN + cft20 + lextale + motiv + start_learning + lang + valency:lang, data = article))
```

```{r RQ3}
summary(lm(medianDurAll ~ L1 + word_length + word_frequency + punctuation + 
             valency + valency:L1, data = article))
```

```{r feature_weights}
weights <- lm(medianL1L2 ~ word_length + word_frequency + punctuation + valency + exposure_EN + 
          cft20 + lextale + motiv + start_learning, data = article)
