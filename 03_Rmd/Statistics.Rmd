...
```{r}
cat("Slowest reader: ", 
    aggregated_dur$participant_id[aggregated_duration$mean_duration == max(aggregated_duration$mean_duration)], ",",
    round(max(aggregated_duration$mean_duration)),"ms", "\n")

cat("Fastest reader: ", 
    aggregated_duration$participant_id[aggregated_duration$mean_duration == min(aggregated_duration$mean_duration)], ",",
    round(min(aggregated_duration$mean_duration)), "ms")
```

The following density plot shows the mean reading times for each participant split between L1 and L2.
```{r density}
L1L2_mean <- aggregated_duration %>%
  group_by(L1) %>%
  summarize(L1L2_mean_dur = mean(mean_duration))

ggplot(aggregated_duration, aes(x = mean_duration, fill = L1)) +
