...
### Participants
```{r mean_participant}
df2 <- article %>% group_by(participant_id) %>% summarize(mean_duration = mean(dur), L1, .groups = "drop") 
df2 <- unique(df2)
```
...
```{r RQ1}
summary(lm(median_dur ~ word_length + word_frequency + punctuation + valency + exposure_EN + 
             cft20 + lextale + motiv + start_learning, data = article))
```

Performs better without cft20!
```{r RQ2}
summary(lm(median_dur ~ word_length + word_frequency + punctuation + valency + 
             exposure_EN + cft20 + lextale + motiv + start_learning 
           + median_en + valency:median_en, data = article)) #median_en or lang??
```
