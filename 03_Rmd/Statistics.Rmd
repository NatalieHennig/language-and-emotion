...
article <- article %>%
  group_by(textnr, token, tokenindex, L1) %>%
  mutate(median_L1L2 = median(dur), .after = median_dur)
```
...
RQ1_df <- article %>%
  ...
  select(textnr, tokenindex, token, mean_dur, median_dur, AAPz, AAPz_cat, anew_score, punctuation, word_length, frequency) %>%
  distinct()
```


```{r RQ1} 
summary(lm(median_dur ~ word_length + frequency + punctuation + AAPz, data = RQ1_df))
```
...
```{r RQ2_df, eval=FALSE, include=FALSE}
