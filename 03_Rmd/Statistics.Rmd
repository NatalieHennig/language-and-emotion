...
### Participants
```{r mean_participant}
ind_reading_times <- article %>% 
  group_by(participant_id) %>% 
  summarize(mean_duration = mean(dur), L1, .groups = "drop") %>%
  distinct()
```

```{r}
cat("Slowest reader: ", 
    ind_reading_times$participant_id[ind_reading_times$mean_duration == max(ind_reading_times$mean_duration)], ",",
    round(max(ind_reading_times$mean_duration)),"ms", "\n")

cat("Fastest reader: ", 
    ind_reading_times$participant_id[ind_reading_times$mean_duration == min(ind_reading_times$mean_duration)], ",",
    round(min(ind_reading_times$mean_duration)), "ms")
```

The following density plot shows the mean reading times for each participant split between L1 and L2.
```{r density}
group_mean <- ind_reading_times %>%
  group_by(L1) %>%
  summarize(grp.mean = mean(mean_duration))

ggplot(ind_reading_times, aes(x = mean_duration, fill = L1)) +
...
ggplot(article, aes(x = cor_valency, fill = L1)) +
  geom_density(alpha = 0.5) +
...
```{r}
ggplot(article, aes(x = lang, y = cor_valency, fill = lang)) +
  geom_boxplot() +
  scale_fill_manual(values = terrain.colors(9)) +
...
When taking the median of all, *exposure*, *start_learning*, etc. doesn't makes sense.
```{r RQ1}
...
```{r RQ3}
summary(lm(median_L1L2 ~ L1 + word_length + word_frequency + punctuation +
             exposure_EN + cft20 + lextale + motiv + start_learning +
             valency + valency:L1, data = article))
```

```{r feature_weights}
weights <- lm(median_L1L2 ~ word_length + word_frequency + punctuation + valency + exposure_EN + 
