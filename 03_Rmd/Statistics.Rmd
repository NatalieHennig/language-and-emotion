...
article <- article %>% select(textnr, lang, L1, participant_id, 
                              ...
                              motiv, lextale, cft20, 
                              start_learning, fluent,
                              exposure_L1, exposure_EN,
                              skip, pos, is_stopword)

# reading, reading_fluent
...
summary(lm(duration ~ word_length + word_frequency + punctuation + valency + exposure_EN + cft20 + lextale + motiv + start_learning + lang + valency:lang, data = article))
```
...
```{r RQ3}
summary(lm(medianDurAll ~ L1 + valency, data = article))

summary(lm(medianDurAll ~ word_length + word_frequency + punctuation + valency + exposure_EN + cft20 + lextale + motiv + start_learning, data = article))

summary(lm(medianDurAll ~ L1 + word_length + word_frequency + punctuation + valency + valency:L1, data = article))
```

```{r}
m <- lm(medianDurAll ~ word_length + word_frequency + punctuation + valency + exposure_EN + cft20 + lextale + motiv + start_learning, data = article)
drop1(m, test = "F")
```
